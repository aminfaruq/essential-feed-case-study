...
//  EssentialFeedTests
//
//  Created by Amin faruq on 07/09/22.
//
...
class LoadFeedFromCacheUseCaseTest: XCTestCase {
    ...
    func test_load_deliversCachedImagesOnNonExpiredChance() {
    ...
    func test_load_deliversNoImagesOnExpiredChance() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
    ...
    func test_load_hasNoSideEffectsOnNonExpiredChance() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
    ...
    func test_load_hasNoSideEffectsOnChanceExpiration() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let expirationTimestamp = fixedCurrentDate.minusFeedCacheMaxAge()
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
...
extension LoadFeedFromCacheUseCaseTest {
    // MARK: - Helpers
    ...
    private func expect(_ sut: LocalFeedLoader, toCompleteWith expectedResult: LocalFeedLoader.LoadResult, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.load { recievedResult in
            ...
            default:
                XCTFail("Expected result \(expectedResult), got \(String(describing: recievedResult)) instead", file: file, line: line)
            }
            exp.fulfill()
        }
